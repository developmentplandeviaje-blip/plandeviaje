...
const StatusBadge = ({ status }) => {
    ...
    return (
        <span
            className={`inline-block px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap shadow-sm transition-all duration-300 ${isAccepted
                            ...
                            {renderPageNumbers().map((n, i) =>
                                n === '...' ? (
                                    ...
                                    <button
                                        key={n}
                                        onClick={() => setPage(n)}
                                        className={`w-8 h-8 rounded-lg text-xs font-bold transition-all duration-200 shadow-sm active:scale-95 ${page === n
